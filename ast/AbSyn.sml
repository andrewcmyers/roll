structure AbSyn = struct
  ...
  datatype exp = Int_e of int
               ...
               | Union_e of exp*exp      (* ++ *)
               | NTimes_e of exp*exp     (* # *)
               | Compare_e of intcomp    (* compare e1 op e2 *)
  ...
  fun subst(v,id,e2):exp =
    case e2 of
       ...
       | Mod_e(x,y) => Mod_e(subst(v,id,x),subst(v,id,y))
       | Compare_e(x,oper,y) => Compare_e(subst(v, id, x), oper, subst(v, id, y))
